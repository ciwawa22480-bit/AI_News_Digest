...
# ============ 去重：标题归一化 + 模糊语义去重 ============
# 说明：
#   旧逻辑只按「URL 完全相同」或「标题前 20 字完全相同」和昨天一天比对，
#   同一件事换个来源、换种措辞（中英/大小写/标点差异）就会绕过，导致
#   今天和昨天大量同义重复。这里升级为：
#     1) 标题归一化（去标点/空格/emoji，大小写统一，抹平常见站点后缀）；
#     2) 字符 2-gram Jaccard + difflib 序列相似度双判据的模糊比对；
#     3) 当天池内互相去重（dedup_within_pool）；
#     4) 与最近 N 天的日报比对做跨天去重（deduplicate_cross_day）。

# 抓取源常见的站点/栏目后缀噪声，归一化时抹掉，避免影响相似度判断
_TITLE_TAIL_NOISE = [
    "google news", "the verge", "techcrunch", "venturebeat", "cnbeta",
    "36kr", "36氪", "钛媒体", "量子位", "it之家", "爱范儿",
]


def _norm_title(t):
    """标题归一化：小写、去除标点/空格/emoji，仅保留中英文与数字。"""
    if not t:
        return ""
    s = t.lower()
    for noise in _TITLE_TAIL_NOISE:
        s = s.replace(noise, "")
    # 仅保留 CJK 汉字、拉丁字母、数字
...
def _make_sig(title):
    """把标题转成 (归一化串, 2-gram 集合) 签名。"""
    norm = _norm_title(title)
    return norm, _shingles(norm, 2)


def _titles_similar(sig_a, sig_b, threshold=0.72):
    """判断两个标题签名是否指向同一件事。多判据取其一命中即算重复。"""
    na, sa = sig_a
    nb, sb = sig_b
    if not na or not nb:
        return False
    ...
    if na == nb:
        return True
    # 2) 一方是另一方的子串（长度足够，避免短词误伤）
    shorter, longer = (na, nb) if len(na) <= len(nb) else (nb, na)
    if len(shorter) >= 8 and shorter in longer:
        return True
    # 3) 字符 2-gram Jaccard
    if _jaccard(sa, sb) >= 0.55:
        return True
    # 4) difflib 序列相似度
...
def load_config_dedup(config):
    ...
    return {
        ...
        "lookback_days": int(d.get("lookback_days", 5)),
        "title_threshold": float(d.get("title_threshold", 0.72)),
        "within_day": d.get("within_day", True),
    }


def dedup_within_pool(items, threshold=0.72):
    """当天池内近重复去重：同义/同事件只保留第一条（保留输入顺序=保留高优先来源）。"""
    kept = []
    kept_sigs = []
    kept_urls = set()
    removed = 0
    for item in items:
        ...
        sig = _make_sig(item.get("title", ""))
        if url and url in kept_urls:
            removed += 1
            continue
        dup = False
        for ks in kept_sigs:
            if _titles_similar(sig, ks, threshold):
                dup = True
                break
        if dup:
            removed += 1
        ...
        if url:
            kept_urls.add(url)
    if removed > 0:
        print("  [INFO] Removed " + str(removed) + " items (near-duplicate within today's pool)")
    return kept


def load_recent_signatures(lookback_days=5):
    """收集最近 N 天日报里的 URL 与标题签名，用于跨天去重。"""
    ...
    for i in range(1, lookback_days + 1):
        ...
        f = os.path.join(DAILY_DIR, day.strftime("%Y-%m-%d") + ".json")
        ...
        try:
            ...
            for item in data.get("items", []):
                ...
                if title:
                    sigs.append(_make_sig(title))
        except Exception:
            continue
    return urls, sigs


def deduplicate_cross_day(items, lookback_days=5, threshold=0.72):
    """跨天去重：与最近 N 天日报比对，URL 相同或标题模糊相似即视为已报道过。"""
    recent_urls, recent_sigs = load_recent_signatures(lookback_days)
    if not recent_urls and not recent_sigs:
        return items
    filtered = []
    for item in items:
        url = (item.get("url") or "").strip()
        ...
        sig = _make_sig(item.get("title", ""))
        dup = False
        for rs in recent_sigs:
            if _titles_similar(sig, rs, threshold):
                dup = True
                break
        if dup:
            continue
    ...
    if removed > 0:
        print("  [INFO] Removed " + str(removed)
              + " items (already covered in last " + str(lookback_days) + " days)")
    return filtered


def deduplicate_from_yesterday_compat(items, config=None):
    """统一去重入口：先做当天池内近重复去重，再做最近 N 天跨天去重。
    参数由 config.dedup 控制，缺省时保持稳健默认值。"""
    cfg = load_config_dedup(config)
    if not cfg["enabled"]:
        return items
    th = cfg["title_threshold"]
    n = len(items)
    if cfg["within_day"]:
        items = dedup_within_pool(items, threshold=th)
    items = deduplicate_cross_day(items, lookback_days=cfg["lookback_days"], threshold=th)
    print("  [INFO] Dedup pool " + str(n) + " -> " + str(len(items))
          + " (within-day + last " + str(cfg["lookback_days"]) + "d)")
    return items
...
def recent_titles_for_prompt(lookback_days=5, limit=80):
    """取最近 N 天日报的中文/英文标题清单，用于提示 AI 跨天去重。"""
    beijing = timezone(timedelta(hours=8))
    today = datetime.now(beijing)
    titles = []
    for i in range(1, lookback_days + 1):
        ...
        try:
            ...
            continue
    # 去掉重复标题，最多保留 limit 条，避免 prompt 过长
    seen = set()
    uniq = []
    for t in titles:
        k = _norm_title(t)
...
def build_ai_prompt(items, config, max_curated, recent_titles=None):
    items_text = ""
    ...
    if recent_titles:
        ...
        recent_block = (
            "## 近期已报道事件（最近几天日报已写过，请勿重复选入语义相同的事件）\n"
            + lines + "\n---\n")
...
def curate_with_ai(items, config):
    ...
    dedup_cfg = load_config_dedup(config)
    recent_titles = recent_titles_for_prompt(
        lookback_days=dedup_cfg.get("lookback_days", 5)) if dedup_cfg.get("enabled", True) else []
    prompt = build_ai_prompt(candidates, config, max_curated, recent_titles=recent_titles)
...
def main():
    ...
    items = deduplicate_from_yesterday_compat(items, config)
    ...
    # 较昨日（上一份日报）新增条目数，用于页面「较昨日新增 N 条」标记
    dedup_cfg = load_config_dedup(config)
    new_vs_yesterday, compare_date = compute_new_vs_yesterday(
        news_items, threshold=dedup_cfg.get("title_threshold", 0.72))
    ...
    output = {
        ...
        "new_vs_yesterday": new_vs_yesterday,
